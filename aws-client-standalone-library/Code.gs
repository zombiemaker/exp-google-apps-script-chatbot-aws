...
Modified version of https://github.com/smithy545/aws-apps-scripts

This version of the script is for use cases where you want the script to be in a separate, re-usable library. This allows you to reuse the code in multiple Apps Script projects.

Create a separate Google Apps Script Project from your main project to be used as a library project and copy this script into the project.

From the Apps Script IDE for the library project, select File > Project properties.  On the "Info" tab, copy the Script ID property value.

After creating a separate Google Apps Script project where you place this script, you create a separate Google Apps Script Project for your main project where you want to include this script as a library.

From the Apps Script IDE for the main project, select Resources > Libraries.

Pase the Script ID into the "Add a library" field and select "Add"

Select the "Title" field for the added library and type the variable name (e.g., "AWS") that you want to use to refer to the script object from within your main script code.

Select the version of the script that you want to use.
...
return {
  ...
  request: function({service, region, action, params, method, payload, headers, uri, options}) {
    ...
    if (action !== 'ListBuckets') {
      Logger.log("action " + action + " does not equal to \'ListBuckets\'");
    }
    else {
      Logger.log("action " + action + " does equal to \'ListBuckets\'");
    }
    
    
    if (service == "s3" && action !== 'ListBuckets' && bucket == undefined) {
